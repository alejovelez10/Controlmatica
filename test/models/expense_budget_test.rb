...
class ExpenseBudgetTest < ActiveSupport::TestCase
  ...
  # Los gastos de las fixtures nacen SIN aceptar y eso no reserva nada: para
  # Controlmatica siguen siendo plata libre hasta que alguien los acepta. Lo que
  # SI sale del tope es lo aceptado sin partida que lo cubra, y de eso se ocupa
  # test/services/expense_budget_service_cap_test.rb.
  def test_tope_no_reserva_los_gastos_sin_aceptar
    assert ReportExpense.where(cost_center_id: @centro.id, is_acepted: false).exists?,
           "Las fixtures deben dejar gastos sin aceptar en el centro"

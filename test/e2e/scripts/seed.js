...
// `scope` acota la siembra a un solo centro de costo (E2E_SCOPE del seed).
// Los specs que MUTAN datos resiembran solo el suyo en beforeAll: cuesta ~6 s de
// boot de Rails, frente a la alternativa de fallos intermitentes.
function seed({ silent = false, scope = "ALL" } = {}) {
  ...
  execFileSync("bin/rails", ["runner", SEED_FILE], {
    ...
    env: { ...process.env, RAILS_ENV: "test", PWD: RAILS_ROOT, E2E_SCOPE: scope },
  });
}
...
if (require.main === module) {
  seed({ scope: process.env.E2E_SCOPE || "ALL" });
}

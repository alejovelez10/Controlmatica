class SalesOrdersController < ApplicationController
  ...
  def get_sales_order
    if params[:date_desde] || params[:date_hasta] || params[:number_order] || params[:cost_center_id] || params[:state] || params[:description]
      ...
      sales_order = SalesOrder.all.paginate(page: params[:page], :per_page => params[:filter]).to_json( :include => {  :cost_center => { :only =>[:code, :invoiced_state] }, :customer_invoices => { :only =>[:invoice_value, :invoice_date] } })
      sales_orders_total = SalesOrder.all.count

    else
      sales_order = SalesOrder.all.paginate(page: params[:page], :per_page => 10).order(id: :desc).to_json( :include => {  :cost_center => { :only =>[:code, :invoiced_state] }, :customer_invoices => { :only =>[:invoice_value, :invoice_date] } })
      sales_orders_total = SalesOrder.all.count
    end
